...
namespace Wmi
{
    ...
    struct Win32_PhysicalMemory
    {
        ...
        void setProperties(const WmiResult& result, std::size_t index)
        {
            // Defined in System Management BIOS (SMBIOS) Reference
            static const std::map<uint32_t, const char*> kMemoryTypes = {
                ...
                {0x1C, "LPDDR2"}, {0x1D, "LPDDR3"},  {0x1E, "LPDDR4"}, {0x1F, "Logical non-volatile device"},
            ...
            result.extract(index, "CreationClass", (*this).CreationClassName);
            ...
            if (kMemoryTypes.find(SMBIOSMemoryType) != std::end(kMemoryTypes))
            {
                MemoryTypeName = kMemoryTypes.at(SMBIOSMemoryType);
            }
            else
            {
    ...
    struct Win32_SMBIOSMemory
    {
        ...
        uint16_t    ErrorAccess;
        uint64_t    ErrorAddress;
        bool        ErrorCleared;
        uint16_t    ErrorDataOrder;
        std::string ErrorDescription;
        uint16_t    ErrorInfo;
        std::string ErrorMethodology;
        uint64_t    ErrorResolution;
        uint32_t    ErrorTransferSize;
        uint32_t    LastErrorCode;
        std::string Name;
        uint64_t    NumberOfBlocks;
        std::string OtherErrorDescription;
